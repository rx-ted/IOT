...
# 44 "../../../../../../src/timer.c"
static rt_uint8_t _soft_timer_status = 1;

static rt_list_t _soft_timer_list[1];
static struct rt_thread _timer_thread;
__attribute__((aligned(8)))
static rt_uint8_t _timer_thread_stack[512];
...
static __inline void _timer_remove(rt_timer_t timer)
{
    int i;
...
rt_err_t rt_timer_start(rt_timer_t timer)
{
    ...
    for (row_lvl = 0; row_lvl < 1; row_lvl++)
    {
             ...
             row_head[row_lvl] = row_head[row_lvl]->next)
        {
            ...
            else if ((t->timeout_tick - timer->timeout_tick) < 0xffffffffUL / 2)
            {
                break;
            }
        }
    ...
    if (timer->parent.flag & 0x4)
    {
        ...
        if ((_soft_timer_status == 1) &&
           ((_timer_thread.stat & 0x04) == 0x04))
        {
...
void rt_timer_check(void)
{
    ...
    while (!rt_list_isempty(&_timer_list[1 - 1]))
    {
        t = ((struct rt_timer *)((char *)(_timer_list[1 - 1].next) - (unsigned long)(&((struct rt_timer *)0)->row[1 - 1])))
                                                                             ;
        ...
        if ((current_tick - t->timeout_tick) < 0xffffffffUL / 2)
        {
            ...
            rt_list_insert_after(&list, &(t->row[1 - 1]));
...
rt_tick_t rt_timer_next_timeout_tick(void)
{
    rt_tick_t next_timeout = 0xffffffffUL;
...
void rt_soft_timer_check(void)
{
    ...
    while (!rt_list_isempty(&_soft_timer_list[1 - 1]))
    {
        t = ((struct rt_timer *)((char *)(_soft_timer_list[1 - 1].next) - (unsigned long)(&((struct rt_timer *)0)->row[1 - 1])))
                                                                               ;
...
static void _timer_thread_entry(void *parameter)
{
    ...
    while (1)
    {
        ...
        if (_timer_list_next_timeout(_soft_timer_list, &next_timeout) != 0)
        {
        ...
        else
        {
            ...
            if ((next_timeout - current_tick) < 0xffffffffUL / 2)
            {
