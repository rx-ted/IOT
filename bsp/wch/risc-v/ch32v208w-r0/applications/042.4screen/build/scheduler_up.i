...
static struct rt_thread* _scheduler_get_highest_priority_thread(rt_ubase_t *highest_prio)
{
    ...
    highest_priority_thread = ((struct rt_thread *)((char *)(rt_thread_priority_table[highest_ready_priority].next) - (unsigned long)(&((struct rt_thread *)0)->tlist)))
...
void rt_schedule(void)
{
    ...
    if (rt_scheduler_lock_nest == 0)
    {
        ...
        if (rt_thread_ready_priority_group != 0)
        {
            ...
            if ((rt_current_thread->stat & 0x07) == 0x03)
            {
                if (rt_current_thread->current_priority < highest_ready_priority)
                {
                    to_thread = rt_current_thread;
                }
                else if (rt_current_thread->current_priority == highest_ready_priority && (rt_current_thread->stat & 0x08) == 0)
                {
                    to_thread = rt_current_thread;
                }
                else
            ...
            if (to_thread != rt_current_thread)
            {
                ...
                rt_schedule_remove_thread(to_thread);
                to_thread->stat = 0x03 | (to_thread->stat & ~0x07);
                ...
                if (rt_interrupt_nest == 0)
                {
                    ...
                    do {if ((rt_scheduler_switch_hook) != 0) rt_scheduler_switch_hook (from_thread); } while (0);
...
# 322 "../../../../../../src/scheduler_up.c"
            ...
            else
            {
                ...
                rt_current_thread->stat = 0x03 | (rt_current_thread->stat & ~0x07);
            }
        }
    }
...
void rt_schedule_insert_thread(struct rt_thread *thread)
{
    ...
    {
        thread->stat = 0x03 | (thread->stat & ~0x07);
        goto __exit;
    }


    thread->stat = 0x02 | (thread->stat & ~0x07);

    if((thread->stat & 0x08) != 0)
    {
